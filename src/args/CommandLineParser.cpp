...
#define SERVER_SOURCE_DEFAULT "https://downloads.nordcdn.com/configs/archives/servers/ovpn.zip"
...
NVPNOptions CommandLineParser::parse(int argc, char *argv[]) {
    ...
    options.add_options()("s,server", "Server to connect", cxxopts::value<std::string>())("c,countries", "List of countries to connect, comma separated", cxxopts::value<std::vector<std::string>>())(
            "h,help", "Print help")("o,ovpn", "Url to retrieve openvpn server configuration", cxxopts::value<std::string>()->default_value(SERVER_SOURCE_DEFAULT))("u,user", "User",
            cxxopts::value<std::string>())("p,password", "Password", cxxopts::value<std::string>());
    cxxopts::ParseResult result = options.parse(argc, argv);
    if (result.count("help")) {
        std::cout << options.help() << std::endl;
        exit(0);
    }
    return NVPNOptions(get_server(result), get_countries(result), get_source(result), get_user(result), get_password(result));
}
...
std::string CommandLineParser::get_source(const cxxopts::ParseResult &result) const {
